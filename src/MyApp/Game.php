...
class Game implements MessageComponentInterface
{
	...
	private $walkableTiles = [
		...
		[4, 6], [4, 7], [4, 8], [4, 9], // Normal grass
		[5, 6], [5, 7], [5, 8], [5, 9], // Normal grass with different patterns
    ];
	...
	private function updatePlayerCharacterIndex($playerId, $characterIndex)
	{
		if (isset($this->players[$playerId])) {
			...
			$stmt = $this->db->prepare("UPDATE players SET character_index = :character_index WHERE id = :id");
	...
	private function createNewPlayer($playerId, ConnectionInterface $conn, $characterIndex = 0)
	{
		...
		$stmt = $this->db->prepare("INSERT INTO players (id, x, y, width, height, character_index, facing, frame, map_id) VALUES (:id, :x, :y, :width, :height, :character_index, :facing, :frame, :map_id)");
        $stmt->bindValue(":id", $playerId, \PDO::PARAM_STR);
	...
	private function movePlayer($playerId, $newX, $newY, $facing, $frame = 0)
	{
		if (isset($this->players[$playerId])) {
			...
			if ($transitionData !== null) {
				...
				$stmt = $this->db->prepare("UPDATE players SET map_id = :map_id, x = :x, y = :y WHERE id = :id");
				...
				if ($mapData !== null) {
					...
					error_log("Map data is null for map ID: " . $player["map_id"]);
				}
			...
			if ($currentTime - $this->lastBroadcastTime >= 0.05) { // Adjust the interval as needed
	...
	private function handleStandardMovement($playerId, $newX, $newY, $facing, $frame) {
		...
		foreach ($this->players as $otherId => $otherPlayer) {
			if ($otherId !== $playerId && $this->isColliding($newX, $newY, $player["width"], $player["height"], $otherPlayer["x"], $otherPlayer["y"], $otherPlayer["width"], $otherPlayer["height"])) {
				$collision = true;
				break;
			}
		}
		...
		if (!$collision && $this->isCollidingWithMap($newX, $newY, $player["width"], $player["height"])) {
			$collision = true;
		}
		...
		if (!$collision) {
			...
			$stmt = $this->db->prepare("UPDATE players SET x = :x, y = :y, facing = :facing, frame = :frame WHERE id = :id");
	...
	private function checkMapTransitions($player, $newX, $newY)
	{
		...
		if ($newX < 0) {
			return $this->handleTransition($currentMapId, 'map_left', ($mapWidth - 1) * $tileSize - $player["width"], $newY);
		} elseif ($newX + $player["width"] > $mapWidth * $tileSize) {
			return $this->handleTransition($currentMapId, 'map_right', 0, $newY);
		}

		if ($newY < 0) {
			return $this->handleTransition($currentMapId, 'map_top', $newX, ($mapHeight - 1) * $tileSize - $player["height"]);
	...
	{
		$stmt = $this->db->prepare("SELECT $directionField FROM maps WHERE id = ?");
	...
	private function isCollidingWithMap($x, $y, $width, $height)
	{
		...
		if ($x < 0 || $x + $width > $mapWidth * $tileSize || $y < 0 || $y + $height > $mapHeight * $tileSize) {
			return true;
		}
	...
	private function findNonOverlappingPosition($width, $height)
	{
		...
		while ($attempt < $maxAttempts) {
			...
			foreach ($this->players as $player) {
				if ($this->isColliding($x, $y, $width, $height, $player["x"], $player["y"], $player["width"], $player["height"])) {
					$overlaps = true;
					break;
				}
			}
		...
		return ["x" => ($mapWidth - $width) / 2, "y" => ($mapHeight - $height) / 2];
	}
	...
	public function onClose(ConnectionInterface $conn)
	{
		...
		foreach ($this->players as $playerId => $player) {
			if ($player["connection"] === $conn) {
				...
				$this->removeFromMap($playerId, $player["map_id"]);
				break;
			}
		}
	}
	...
	{
		if ($e instanceof \RuntimeException && strpos($e->getMessage(), 'Unable to write to stream') !== false) {
